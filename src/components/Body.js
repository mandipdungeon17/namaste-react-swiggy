...
const Body = () => {
    ...
    */
    ...
    const getTopRatedRest = () => {
      const filteredList = listOfRestaurant.filter((rest) => rest.info.avgRating > 4.2);
      setFilterRestaurant(filteredList);
    };

    const getSearchedRest = () => {
      const searchedRest = listOfRestaurant.filter((rest) => rest.info.name.toLowerCase().includes(searchText.toLowerCase()));
      searchedRest.length === 0 ? setFilterRestaurant(listOfRestaurant) : setFilterRestaurant(searchedRest);
      setSearchText("");
    }
